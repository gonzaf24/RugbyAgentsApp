...
export const EditData = ({ player, level, onSubmitPlayer, loadingConfirm }) => {
  ...
  const [disableAgregarHistoryClub, setDisabledAgregarHistoryClub] = useState(
    true
  );
  const [disableAgregarNacionalidad, setDisabledAgregarNacionalidad] = useState(
    true
  );
  ...
  const onSelectFilePortada = async (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const fileName = uniqid() + "-" + user.uid + "." + ext;
  ...
  const onSelectFiles = async (e) => {
    ...
    if (e.target.files && e.target.files.length > 0) {
      for (var i = 0; i < e.target.files.length; i++) {
        ...
        const fileName = nombre + "--" + uniqid() + "-" + user.uid + "." + ext;
  ...
  const submitConfirm = async (event) => {
    ...
    try {
      ...
      onSubmitPlayer({
        ...
        uid: uid,
        nivel: nivel,
        idiomasResponsable: idiomasResponsable,
        ...
        return (
          <>
            <form className={classes.form} onSubmit={submitDatosDeportivos}>
              ...
              <div className={classes.nacionalidades}>Descripcion</div>
